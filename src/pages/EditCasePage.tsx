...
import { getCaseById, updateCaseDetails } from '../features/cases';
...
export default function EditCasePage() {
  ...
  useEffect(() => {
    ...
    document.title = 'MedViz - Edit Case';
  }, []);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const updatedCase = await updateCaseDetails(caseId, {
        title,
        description,
      });
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Failed to update the clinical case.');
    } finally {
    ...
    return (
      <div className="flex min-h-full items-center justify-center px-4">
        <div className="rounded-[30px] border border-medviz-line bg-[rgba(15,39,69,0.9)] px-8 py-10 text-center shadow-[0_24px_60px_rgba(3,10,18,0.4)] backdrop-blur">
          ...
          <h1 className="mt-4 font-display text-3xl font-bold text-medviz-ink">Loading case details</h1>
          <p className="mt-3 text-sm leading-6 text-medviz-ink/68">
            Preparing the case information for editing.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-full bg-transparent">
      <SiteHeader
        actions={[
          { label: 'Back to Case', to: `/cases/${caseId}`, variant: 'outline' },
        ]}
      />
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <h1 className="mt-2 font-display text-4xl font-bold text-medviz-ink">Update case details</h1>
                <p className="mt-3 max-w-2xl text-sm leading-7 text-white/68">
                  Revise the case title or notes without changing the saved patient model.
                </p>
              </div>
            </div>
        ...
        <form className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]" onSubmit={handleSubmit}>
          <section className="rounded-[34px] border border-medviz-line/80 bg-[rgba(15,39,69,0.88)] px-6 py-7 shadow-[0_24px_70px_rgba(3,10,18,0.4)] backdrop-blur lg:px-8">
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-7 text-white/68">
                  Keep the title clear and the notes concise so the case remains easy to review later.
                </p>
              </div>
          ...
          <aside className="rounded-[34px] border border-medviz-line/80 bg-[linear-gradient(160deg,rgba(9,22,39,0.98),rgba(15,39,69,0.96))] px-6 py-7 text-white shadow-[0_30px_80px_rgba(3,10,18,0.45)] lg:px-8">
            <div>
              ...
              <h2 className="mt-3 font-display text-3xl font-bold">Save your changes</h2>
              <p className="mt-4 text-sm leading-7 text-white/74">
                The patient model stays attached to the case. Only the case title and notes will change here.
              </p>
            </div>

            <div className="mt-8 rounded-[24px] border border-white/14 bg-white/8 p-5 backdrop-blur-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/52">What changes</p>
              <p className="mt-3 text-sm leading-6 text-white/74">
                Case name and notes can be updated at any time from this screen.
              </p>
            </div>

            <div className="mt-8 border-t border-white/10 pt-6">
              <div className="text-sm text-white/58">
                {isSubmitting ? 'Saving updates...' : 'Save the case details when you are ready.'}
              </div>
